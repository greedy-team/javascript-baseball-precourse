...
[출력 관련 기능]
- 형식에 맞지 않는 값을 입력할 시 `alert`를 사용하여 사용자에게 에러메시지를 보여주는 기능
- 게임의 결과를 나타내는 기능(정답을 맞추지 못한 경우)
- 게임의 결과를 나타내는 기능2(정답을 맞춘 경우)

[로직 관련 기능]
- 게임의 결과를 판별하는 '로직(play 메서드)' 생성(스트라이크, 볼, 낫싱 판별)
- '확인' 버튼 클릭 시 결과를 판별하는 로직을 실행하는 기능

[요구사항]
- import문을 이용해 스크립트를 모듈화하기
~