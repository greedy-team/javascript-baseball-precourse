...
## 구현할 기능 목록
필요 클래스
1. BaseBallGame - 숫자 야구 게임의 실행 로직을 작성할 클래스
...
3. getRandomNumbers - 램덤한 값을 MissionUtils 라이브러리를 사용하여 설정하여 반환할 함수

필요 기능
1. 첫 초기화 - 페이지 접속시 정답을 MissionUtils 라이브러리를 사용하여 랜덤 설정
2. 확인 버튼 클릭 이벤트 - 버튼 클릭시 userinput에서 값을 가져와 BaseBallGame의 play 함수를 이용하여 정답과 비교
3. play 함수의 반환 값에 따른 출력 - 반환 값에 따라 어떤것을 출력 할지 결정
4. 게임 재시작 기능 - 정답을 맞췄다면 정답 값을 다시 랜덤 설정 

## Mission2
## 해야할 일
1. 기존에 index.js 파일에 view 관련 코드가 모여있는 것을 분리하여 클래스 작성
2. additionalFunctions에 있는 함수들을 BaseballGame 클래스에 이동
3. 함수들과 변수들이 각 자 알맞는 위치에 있는지 확인하고 이동하기
